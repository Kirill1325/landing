...
import { toggleRepairModal } from '../model/modalSlice'
...
export const RepairModal = () => {
    ...
    const { repairModal } = useAppSelector(state => state.modalSlice)
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { resetForm }) => {
            ...
            resetForm();
            dispatch(toggleRepairModal())
            dispatch(toggleSubmittedModal())
        },
    });

    return (
        <div className={`${cl.modal} ${repairModal ? cl.open : ''}`} onClick={() => dispatch(toggleRepairModal())}>
            <div className={cl.modalContent} onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <label>Техника: </label>
                    <Dropdown values={['Смартфон', 'Другой Смартфон', 'Еще Смартфон']} />

                    <div className={cl.twoDropdowns}>
                        <div className={cl.dropdown1}>
                            <label>Бренд:</label>
                            <Dropdown values={['Apple', 'Samsung', 'Xiaomi', 'Huawei']} />
                        </div>
                        <div className={cl.dropdown1}>
                            <label>Модель:</label>
                            <Dropdown values={['IPhone 12', 'IPhone 13', 'IPhone 14']} />
                        </div>
