...
import { toggleBasicModal } from '../model/modalSlice'
...
export const BasicModal = () => {
    ...
    const { basicModal } = useAppSelector(state => state.modalSlice)
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { resetForm }) => {
            ...
            dispatch(toggleBasicModal())
        },
    });

    return (
        <div className={`${cl.modal} ${basicModal ? cl.open : ''}`} onClick={() => dispatch(toggleBasicModal())}>
            <div className={cl.modalContent} onClick={e => e.stopPropagation()}>

                <div className={cl.modalHeader}>
                    <p>Заполните форму и с вами свяжутся  в ближайшее время</p>
                    <button onClick={() => dispatch(toggleBasicModal())}>
                ...
                <form onSubmit={formik.handleSubmit}>
                    ...
                    {formik.touched.comment && formik.errors.comment ? (
                        <div className={cl.error}>{formik.errors.comment}</div>
                    ) : null}
